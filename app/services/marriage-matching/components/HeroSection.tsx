...
import { Check, Star, Lock, Phone, ClipboardList } from "lucide-react";
...
const benefits = [
  ...
  "Marriage Strength Analysis",
  "Personalized Remedies",
];

const trustBadges = [
  { text: "Trusted Astrological Guidance", icon: <Star className="w-4.5 h-4.5 text-amber-400 fill-amber-400 shrink-0" /> },
  { text: "100% Confidential", icon: <Lock className="w-4.5 h-4.5 text-emerald-400 shrink-0" /> },
  { text: "Personal Consultation", icon: <Phone className="w-4.5 h-4.5 text-sky-400 shrink-0" /> },
  { text: "Detailed Analysis", icon: <ClipboardList className="w-4.5 h-4.5 text-indigo-400 shrink-0" /> },
];
      ...
      <div className="mm-hero-inner">
        <div className="mm-hero-text">
          ...
          <h1 className="mm-hero-h1">
            Find Your Perfect Life Partner with <span className="mm-gold">Accurate Horoscope Matching</span>
          </h1>
          <p className="mm-hero-sub">
            Ensure a happy and harmonious married life through detailed horoscope compatibility analysis. Our expert astrologer checks Porutham, Doshas, Dasha compatibility, and overall marital strength before marriage.
          </p>

          <div className="mm-benefits-grid">
            {benefits.map((benefit, i) => (
              <div key={i} className="mm-benefit-item">
                <div className="mm-benefit-icon">
                  <Check className="w-3.5 h-3.5" />
                </div>
                <span className="mm-benefit-text">{benefit}</span>
              </div>
            ))}
          </div>

          <div className="mm-cta-row">
            <a href="#types" className="mm-btn-primary">
          ...
          <div className="mm-trust-grid">
            {trustBadges.map((badge, i) => (
              <div key={i} className="mm-trust-badge-card">
                <div className="mm-trust-badge-icon">
                  {badge.icon}
                </div>
                <span className="mm-trust-badge-text">{badge.text}</span>
              </div>
            ))}
          </div>
        </div>
